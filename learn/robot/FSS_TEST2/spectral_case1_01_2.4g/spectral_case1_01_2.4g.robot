...
${RADIO_EQ}           	radio_id =
${RADIO_EQ_1}           radio_id = 1

*** Test Cases ***
Test bandwidth config and fss config
	[Documentation]    配置频宽，使能radio1的频谱检测
	#****切换到AC配置spectral enable radio 1 并且配置2.4g频宽为40MHZ*********#
	Switch Connection	1
	Write Bare	\r\n
	Read
	Write				chan-width 40 radio 1
	Write				spectral enable radio 1
	${output}=          Execute Command    show ap-config running ${AP_NAME}
	Should Match		${output}          *${AC_TEST_CMD1}*
	Should Match		${output}          *chan-width 40 radio 1*
	Sleep				1s
	...
	Should Match		${output}          *${AC_TEST_CMD1}*
	${output}=          Execute Command    show dot11 wireless 1/0 | i Channel width                    #直接读取radio 1的频宽信息
	...
	#****比较output文件中的radio_id = 1、freq = 2442、ch_width = 82、spectral_rssi = 0、bin_pwr_count = 213*********#
	#radio_id = 1#
	${ret}=	fss_check_string	${output}	${RADIO_EQ}	${RADIO_EQ_1}									#调用python自写库，并传入${output}、'radio ='、'radio_id = 1' 三个字符串作为参数	
	...
	#freq = 2442#
	${ret}=	fss_check_string	${output}	freq =	freq = 2442											#函数传参：字符串不用""或者'' 区别，直接写字符串就可以，用tab分割多个字符串
	Run Keyword If		${ret}==1	log	${ret}
	...	ELSE	Should Match		${output}          *no find freq = 2442*
	
	#ch_width = 82#
	${ret}=	fss_check_string	${output}	ch_width =	ch_width = 82
	Run Keyword If		${ret}==1	log	${ret}
	...	ELSE	Should Match		${output}          *no find ch_width = 82*
	
	#bin_pwr_count = 213#
	${ret}=	fss_check_string	${output}	bin_pwr_count =	bin_pwr_count = 213
	Run Keyword If		${ret}==1	log	${ret}
	...	ELSE	Should Match		${output}          *no find bin_pwr_count = 213*
	
	#spectral_rssi = 0到255#
	${ret}=	fss_check_string_value	${output}	spectral_rssi =
	Run Keyword If		${ret}==1	log	${ret}
	...	ELSE	Should Match		${output}          *no find spectral_rssi = x*
