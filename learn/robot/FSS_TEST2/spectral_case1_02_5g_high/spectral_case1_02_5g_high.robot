...
Test bandwidth config and fss config
	...
	#****切换到AC配置spectral enable radio 2 并且配置5g 157信道频宽为40MHZ*********#
	Switch Connection	1
	Write Bare	\r\n
	Read
	Write				channel 157 radio 2
	Write				chan-width 40 radio 2
	Write				spectral enable radio 2
	Write				device mode monitor radio 2
	...
	Write Bare			\r\n
	Sleep				15s
	${output}=			Read																			#读走前面10秒的打印
	Write				no spectral debug level															#停止打印信息
	Sleep				60s																				#休眠60s，让打印缓存刷到终端
	${output}=			Read																			#读走打印信息，确保5.8G 高频信息被捕获
	#log to console	\n ${output}																		#打印log到终端
	
	#****比较output文件中的radio_id = 2、freq = 5785、ch_width = 100、spectral_rssi = 0、bin_pwr_count = 260*********#
	#radio_id = 1#
	...
	#freq = 5785#
	${ret}=	fss_check_string1	${output}	freq =	freq = 5785											#函数传参：字符串不用""或者'' 区别，直接写字符串就可以，用tab分割多个字符串
	Run Keyword If		${ret}==1	log	${ret}
	...	ELSE	Should Match		${output}          *no find freq = 5785*
	
	#ch_width = 100#
	${ret}=	fss_check_string1	${output}	ch_width =	ch_width = 100
	Run Keyword If		${ret}==1	log	${ret}
	...	ELSE	Should Match		${output}          *no find ch_width = 100*
	
	#bin_pwr_count = 260#
	${ret}=	fss_check_string1	${output}	bin_pwr_count =	bin_pwr_count = 260
	Run Keyword If		${ret}==1	log	${ret}
	...	ELSE	Should Match		${output}          *no find bin_pwr_count = 260*
